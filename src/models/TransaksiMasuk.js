...
class TransaksiMasuk {
  static async findAll(filters = {}) {
    let sql = `
      ...
      LEFT JOIN barang b ON tm.barang_id = b.id
    ...
    if (filters.barangId) {
      paramCount++;
      sql += ` AND tm.barang_id = $${paramCount}`;
      params.push(filters.barangId);
    }
    
    if (filters.startDate) {
      paramCount++;
      sql += ` AND tm.tanggal >= $${paramCount}`;
  ...
  static async create(data, userId) {
    ...
    try {
      ...
      const result = await client.query(`
        INSERT INTO transaksi_masuk (barang_id, jumlah, tanggal, supplier, catatan, created_by)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id
      `, [
        data.barangId,
  ...
  static async count(filters = {}) {
    let sql = 'SELECT COUNT(*) as count FROM transaksi_masuk WHERE 1=1';
    const params = [];
    let paramCount = 0;
